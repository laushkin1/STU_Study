...
void bubbleSort(Weight *data, const size_t length) {
    ...
    for (size_t i = 0; i < length-1; i++){
        for(size_t j = 0; j < length-1-i; j++){
            int data_j = data[j].product + data[j].packing;
            int data_j1 = data[j+1].product + data[j+1].packing;
            if(data_j < data_j1){
                Weight temp = data[j];
                data[j] = data[j+1];
                data[j+1] = temp;
...
size_t partition(int *data, const size_t pivot, const size_t low, const size_t high) {
    if(low > pivot || pivot >= high || low >= high) return 0;
    int tmp;
...
void quickSort(int *data, const size_t low, const size_t high) {
    ...
    size_t pv = getPivotIndex(data, low, high);
    size_t p = partition(data, pv, low, high);

    if (p > low) quickSort(data, low, p);
    if (p + 1 < high) quickSort(data, p + 1, high);
}
...
void printArr(Weight *date, size_t length){
    for(size_t i = 0; i<length; i++){
        cout << '(' << date[i].product << ", " << date[i].packing << ')' << ' ';
    }
    cout << endl;
}
...
int main() {
    ...
    */
    ...
    /* 3.
        ...
        data: (10, 20, 1000, 30, 40),                         low: 2, high: 3 -> return 2
    */
    // int ar[] = {10, 20, 2000, 30, 1000, 40, 5000, 50, 60, 70};
    // size_t n = getPivotIndex(ar, 2, 7);
    // cout << n << endl;
    ...
    /* 4. 
        ...
        vystup: data: (10, 20, 50, 60, 70, 50, 30, 40, 80, 90), return 5
    */
    // int ar[] = {10, 20, 30, 40, 50, 60, 70, 50, 80, 90};
    // cout << partition(arr, 4, 2, 8) << endl;
    // printArr(ar, 10);
